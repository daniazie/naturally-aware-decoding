...
def main(args):
    ...
    def load_dataset(path):
        try:
            ...
            df = pd.read_csv(path)
        # df = df.dropna()
        messages = {"prompt_template_label": [], "messages": []}

        for i, row in df.iterrows():
            for prompt_template_label, prompt_template in enumerate(
                [args.prompt_template_negative, args.prompt_template_positive]
            ):
                    ...
                    [
                        {
                            ...
                            "content": prompt_template.format(
                                input=row[args.prompt_field]
                            ),
                        },
                        {
                            "role": "assistant",
                            "content": row[args.completion_field],
                        },
                    ]
                )
    ...
    def tokenize_func(examples):
        ...
        concated_input_len = [
            len(i) for i in tokenizer.apply_chat_template(examples["messages"])
        ]
    ...
    model_positive = AutoModelForCausalLM.from_pretrained(
        args.model_positive, **args.model_args
    ).to(args.device)
    model_negative = AutoModelForCausalLM.from_pretrained(
        args.model_negative, **args.model_args
    ).to(args.device)
    model_positive.eval()
    model_negative.eval()
    ...
    for path in glob(args.data_path):
        ...
        dataset_for_positive_model.set_format(
            type="torch",
            columns=["input_ids", "score_mask"],
            output_all_columns=True,
        )
        dataset_for_negative_model.set_format(
            type="torch",
            columns=["input_ids", "score_mask"],
            output_all_columns=True,
        )
        ...
        for i in tqdm(
            ...
            batch_for_positive_model = dataset_for_positive_model[
                i : i + args.batch_size
            ]
            batch_for_negative_model = dataset_for_negative_model[
                i : i + args.batch_size
            ]
            inputs_for_positive_model = {
                k: batch_for_positive_model[k].to(args.device)
                for k in ["input_ids", "score_mask"]
            }
            inputs_for_negative_model = {
                k: batch_for_negative_model[k].to(args.device)
                for k in ["input_ids", "score_mask"]
            }
            log_lklh_positive = get_log_lklh(
                model_positive,
                output_average=True,
                **inputs_for_positive_model,
            )
            log_lklh_negative = get_log_lklh(
                model_negative,
                output_average=True,
                **inputs_for_negative_model,
            )
            llr = log_lklh_positive - log_lklh_negative
            results["file_path"].extend(
                [path] * len(batch_for_positive_model["messages"])
            )
            results["messages_for_positive_model"].extend(
                batch_for_positive_model["messages"]
            )
            results["messages_for_negative_model"].extend(
                batch_for_negative_model["messages"]
            )
