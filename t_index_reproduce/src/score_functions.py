...
def featurize_fn(examples, model, tokenizer):
    ...
    for messages in examples["messages"]:
        input_len = len(
            tokenizer.apply_chat_template(messages[:-1], add_generation_prompt=True)
        )
        input_ids = tokenizer.apply_chat_template(
            messages, return_dict=False, return_tensors="pt"
        ).to(model.device)
        outputs = model(input_ids, output_hidden_states=True)
        labels = input_ids[:, input_len:]
        output_feats = [
            hs[:, input_len:, :].mean(dim=1)[-1] for hs in outputs.hidden_states
        ]
...
def score_log_likelihood(**kwargs):
    ...
    for logits, labels in zip(kwargs["logits"], kwargs["labels"]):
        ...
        log_likelihood = (
            logits.log_softmax(dim=-1).gather(dim=-1, index=labels).squeeze(-1)
        )
