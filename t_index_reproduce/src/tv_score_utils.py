...
class OutOfDistInfo:
    def __init__(self, all_hidden_states):
        self.info = all_hidden_states
...
def score_trajectory_volatility_fn(ID_info, **kwargs):
    ...
    for hidden_states in kwargs["hidden_states"]:
        hidden_states = [[x.to(torch.float32).cpu() for x in hidden_states]]
        OODscore = OODScore({"hidden_state": hidden_states}, ID_info)
        tv_score_all, _ = OODscore.get_tv_score(max_order=5)
        scores.append(tv_score_all[0])
    return scores
