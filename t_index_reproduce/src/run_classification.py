...
class DataTrainingArguments:
    ...
    shuffle_train_dataset: bool = field(
        default=False,
        metadata={"help": "Whether to shuffle the train dataset or not."},
    )
    ...
    def __post_init__(self):
        if self.dataset_name is None:
            ...
            if self.train_file.endswith("jsonl"):
                train_extension = "json"
            assert train_extension in [
                "csv",
                "json",
            ], "`train_file` should be a csv or a json file."
            validation_extension = self.validation_file.split(".")[-1]
            if self.validation_file.endswith("jsonl"):
...
def main():
    ...
    def convert_paired_to_single(examples):
        output = {"sentence": [], "label": []}
        for positive_sample in examples["foreignization"]:
            output["sentence"].append(positive_sample)
            output["label"].append(1)
        for negative_sample in examples["domestication"]:
            output["sentence"].append(negative_sample)
            output["label"].append(0)
        return output

    with training_args.main_process_first(desc="dataset map pre-processing"):
        raw_datasets = raw_datasets.map(
            convert_paired_to_single,
            batched=True,
