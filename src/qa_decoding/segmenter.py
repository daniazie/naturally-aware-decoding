...
class Segmenter:
    ...
    def model_forward(self, batch):
        ...
        logits: torch.Tensor = (
            self.model(**forward_inputs, use_cache=True).logits[:, :-1].cpu()
        )
    ...
    def compute(
        ...
        lang: str | None = None,
        batches: List[List[List[dict]]] | List[List[dict]] | None = None,
    ):
        masks = []
        if batches:
        ...
        for i, batch in enumerate(
            tqdm(batches, total=len(prompts), desc="Segmenting...")
        ):
            outputs = self.model_forward(batch)
